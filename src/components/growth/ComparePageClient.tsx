...
export function ComparePageClient({
  ...
  const specificationRows: Array<{
    ...
    { label: "Manufactured", value: (car) => String(car.year) },
    {
      label: "Registered",
      value: (car) => car.registrationYear ? String(car.registrationYear) : "Ask showroom"
    },
    { label: "Mileage", value: (car) => formatMileage(car.mileage) },
    { label: "Body type", value: (car) => car.bodyType || "Ask showroom" },
    { label: "Exterior colour", value: (car) => car.exteriorColor || "Ask showroom" },
    { label: "Seats", value: (car) => car.seats ? String(car.seats) : "Ask showroom" },
    ...
    { label: "Drivetrain", value: (car) => car.drivetrain || "Ask showroom" },
    { label: "Assembly", value: (car) => car.assemblyType || "Ask showroom" },
    { label: "Stock ID", value: (car) => car.stockCode || "Not assigned" },
  ...
  return (
    <main>
      <header className="border-b-8 border-racing bg-ink text-white">
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
          ...
          <div className="mt-8 max-w-3xl">
            <p className="flex items-center gap-2 border-l-8 border-racing pl-3 text-xs font-black uppercase tracking-[0.22em] text-white/70">
              <GitCompareArrows className="h-4 w-4" aria-hidden="true" />
              Your shortlist
            </p>
            <h1 className="mt-5 font-display text-5xl font-black uppercase leading-[0.92] tracking-[-0.025em] sm:text-7xl">
      ...
      <section aria-labelledby="add-vehicle-heading" className="border border-ink/15 bg-white p-4 sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row lg:w-[34rem]">
            <select
              ...
              className="h-12 min-w-0 flex-1 border border-ink/20 bg-white px-4 text-sm font-semibold text-ink outline-none hover:border-ink/35 focus:border-racing focus-visible:ring-2 focus-visible:ring-racing/20 disabled:cursor-not-allowed disabled:bg-smoke disabled:opacity-55"
            >
              <option value="">
              ...
              {availableToAdd.map((car) => (
                <option key={car.id} value={car.id}>
                  {vehicleName(car)} — {car.formattedPrice}
                </option>
              ))}
            </select>
            <button
              ...
              className="inline-flex h-12 shrink-0 items-center justify-center gap-2 bg-racing px-5 text-sm font-black uppercase tracking-[0.06em] text-white transition hover:bg-copper focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-racing focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
            >
        ...
        {message ? <p role="status" aria-live="polite" className="mt-3 text-xs font-semibold text-racing">{message}</p> : null}
      </section>

      {selectedCars.length === 0 ? (
        <section className="mt-8 border border-dashed border-ink/25 bg-white px-5 py-14 text-center">
          <span className="mx-auto grid h-14 w-14 place-items-center border border-ink/15 bg-smoke text-racing">
            <GitCompareArrows className="h-7 w-7" aria-hidden="true" />
          </span>
          <h2 className="mt-4 font-display text-3xl font-black uppercase leading-none text-ink">Start your shortlist</h2>
          ...
          <Link href="/#inventory" className="mt-6 inline-flex min-h-12 items-center justify-center gap-2 bg-racing px-5 text-sm font-black uppercase tracking-[0.06em] text-white transition hover:bg-copper focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-racing focus-visible:ring-offset-2">
            Browse inventory
            <ArrowUpRight className="h-4 w-4" aria-hidden="true" />
          </Link>
        </section>
      ) : (
        <section aria-label="Vehicle comparison" className="mt-8 overflow-hidden border border-ink/15 bg-white">
                ...
                <tr className="align-top">
                  ...
                  {selectedCars.map((car) => (
                    <th key={car.id} scope="col" className="min-w-[245px] border-b border-r border-ink/10 p-4 last:border-r-0">
                      <div className="relative aspect-[16/9] overflow-hidden border border-ink/10 bg-smoke">
                        {car.imageUrl ? (
                          <Image
                        ...
                        <button
                          ...
                          aria-label={`Remove ${vehicleName(car)} from comparison`}
                          className="absolute right-2 top-2 grid h-9 w-9 place-items-center border border-ink/15 bg-white text-ink transition hover:border-racing hover:bg-racing hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-racing"
                        >
                          <Trash2 className="h-4 w-4" aria-hidden="true" />
                        </button>
                      </div>
                      <p className="mt-3 text-xs font-black uppercase tracking-[0.15em] text-racing">{car.year} · {car.brand}</p>
                      <h2 className="mt-1 font-display text-xl font-black uppercase leading-none text-ink">
                        {vehicleModelLabel(car.model, car.variant)}
                      </h2>
                    </th>
                  ))}
                </tr>
                  ...
                  {selectedCars.map((car) => (
                    <td key={car.id} className="border-r border-t border-ink/10 p-4 last:border-r-0">
                      <div className="grid gap-2">
                        <Link href={`/cars/${car.slug}`} className="inline-flex min-h-11 items-center justify-center gap-2 bg-ink px-4 text-xs font-black uppercase tracking-[0.06em] text-white transition hover:bg-racing focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-racing focus-visible:ring-offset-2">
                          View vehicle
                          <ArrowUpRight className="h-4 w-4" aria-hidden="true" />
                        </Link>
                        {car.status !== "SOLD" ? (
                          <Link href={`/book-test-drive?carId=${encodeURIComponent(car.id)}`} className="inline-flex min-h-11 items-center justify-center border border-ink/20 bg-smoke px-4 text-xs font-black uppercase tracking-[0.06em] text-ink transition hover:border-racing hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-racing focus-visible:ring-offset-2">
                            Book test drive
                          </Link>
                        ) : null}
                      </div>
